...
INCORRECT_OUTPUT_DIR = Path(os.path.join(IMG_DIR, "incorrect_august")).resolve()
INCORRECT_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
INCORRECT_DETECTIONS = os.path.join(
    os.path.dirname(os.path.realpath(__file__)),
    MODEL_DIR,
    "vvd_annotations",
    "incorrect_detections.txt",
)


def main(event_id: str, generate_all: bool = True) -> None:
    """_summary_
    ...
    dnb_filename = dnb_url.split("/")[-1]
    geo_filename = geo_url.split("/")[-1]

    dnb_path = os.path.join(IMG_DIR, dnb_filename)

    if not os.path.exists(dnb_path):
        with open(dnb_path, "w+b") as fh:
            utils.download_url(dnb_url, TOKEN, fh)

    geo_path = os.path.join(IMG_DIR, geo_filename)
    if not os.path.exists(geo_path):
        with open(geo_path, "w+b") as fh:
            utils.download_url(geo_url, TOKEN, fh)

    try:
        cloud_maskname = phys_url.split("/")[-1]
        phys_path = os.path.join(IMG_DIR, cloud_maskname)
    ...
    if generate_all:
        detections, status = utils.viirs_annotate_pipeline(
            dnb_filename,
            geo_filename,
            IMG_DIR,
            INCORRECT_OUTPUT_DIR,
        ...
        try:
            ...
            if chip.any():
                ...
                out_img_filename = (
                    f"{product_name}.{year}.{doy}.{time}.{lat}_{lon}.jpeg"
                )
